import {
  INIT_STATE, WEST, EAST, NORTH, SOUTH,
} from './initialState';
...
const _isSnake = (coordinate, snake) => snake.filter((snakeSlice) => _isSamePosition(coordinate, snakeSlice)).length;

const _getSnakeHead = (snake) => snake[0];

const getWillSnakeEating = ({ snake, apple, moves }) => _isSamePosition(
...
const getIsSnakeSilly = (snake) => _isSnake(
  _getSnakeHead(snake),
  _getSnakeTail(snake),
);
...
export const snakeMoveState = (state) => {
  ...
  return {
    ...
    moves: (state.moves.length > 1) ? [...state.moves.slice(0, -1)] : [...state.moves],
  };
};
...
export const snakeChangeDirectionState = (state, move) => {
  switch (move) {
    case 'w':
      if (
        _conditionSnakeChangeDirectionState(state, NORTH)
      ) {
    ...
    case 'a':
      if (
        _conditionSnakeChangeDirectionState(state, WEST)
      ) {
    ...
    case 's':
      if (
        _conditionSnakeChangeDirectionState(state, SOUTH)
      ) {
    ...
    case 'd':
      if (
        _conditionSnakeChangeDirectionState(state, EAST)
      ) {
